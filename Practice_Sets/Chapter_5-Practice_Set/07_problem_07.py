#If the names of 2 friends are the same, then what will happen to the problem 6? 
#If the names of 2 friends are the same, then the second entry will overwrite the first entry in the dictionary. This is because dictionary keys must be unique, and when a duplicate key is added, it will replace the existing value associated with that key. Therefore, only the favourite language of the last friend with that name will be stored in the dictionary. 
